...
class TFBlock(nn.Module):
    ...
    def forward(self, x, mask, return_attn=False): # ADD
        out = self.ln_1(x) if self.norm else x
        if not return_attn: # ADD
            attn_output = self.mha(out, out, out, mask, output_attn=return_attn) # ADD
        else: # ADD
            attn_output, attn_QK_vals = self.mha(out, out, out, mask, output_attn=return_attn) # ADD
        ...
        if self.mlp:
            ...
            x = x + out if self.residual else x
        x = (x, attn_QK_vals) if return_attn else x # ADD
        return x
...
class TFModel(nn.Module):
    ...
    def forward(self, src, return_attn=False):
        ...
        for i, (block) in enumerate(self.h):
            if not return_attn:
                out = block(out, mask, return_attn=return_attn)
            else:
                out, new_info = block(out, mask, return_attn=return_attn)
                info.append(new_info)
        out = self.ln_f(out) if self.output_norm else out
        out = self.fc(out)
        return (out, info) if return_attn else out
